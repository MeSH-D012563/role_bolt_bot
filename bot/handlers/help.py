...
async def cmd_help(message: Message, settings: Settings) -> None:
    ...
    if message.chat.type == "private":
        text = (
            ...
            "Титулы дают разные эффекты: бонусы к играм, /get_cash и защите\n"
            "Если титулов 2+, бот списывает ежедневный налог на титулы\n\n"
            "🏷 Титулы\n"
            "/sell_title <цена> — выставить активный титул на продажу (в группе)\n"
            "/sell_title_now — мгновенно продать активный титул боту за 90% (только ЛС, с подтверждением)\n"
            "/gift_title — подарить активный титул (ответом на сообщение)\n\n"
            "💸 Переводы\n"
            "/pay <сумма> — перевести деньги (ответом на сообщение)\n\n"
            "🎁 Бонус\n"
            f"/get_cash — {settings.daily_cash_amount} {settings.currency} каждые 12 часов\n\n"
            "🛍 Магазин\n"
